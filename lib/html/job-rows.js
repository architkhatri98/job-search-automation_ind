...
const STATE_ABBR = {
  ...
  'Delhi': 'DL', 'New Delhi': 'DL', 'NCR': 'NCR', 'Delhi NCR': 'NCR',
...
function normalizeLocation(loc) {
  ...
  // "India - City, State" or "India - State"
  s = s.replace(/^India\s*[-–,|]\s*/i, '');

  // "State - City" (abbrev) → "City, State" e.g. "KA - Bangalore"
  const abbrDashCity = s.match(/^([A-Z]{2,3})\s*[-–]\s*(.+)$/);
  if (abbrDashCity) s = `${abbrDashCity[2].trim()}, ${abbrDashCity[1]}`;

  // "StateName - City" → "City, ST" e.g. "Karnataka - Bangalore"
  for (const [full, abbr] of Object.entries(STATE_ABBR)) {
    const m = s.match(new RegExp(`^${full}\\s*[-–]\\s*(.+)$`, 'i'));
    if (m) { s = `${m[1].trim()}, ${abbr}`; break; }
  }

  // Strip leading country: "India, City" → "City"
  s = s.replace(/^India\s*[,/]\s*/i, '');
...
function extractSalary(desc) {
  ...
  // Match INR/LPA patterns: "X LPA - Y LPA", "X-Y LPA", "INR X-Y lakhs", "Rs X-Y lakhs"
  const lpaRe = /(?:INR|Rs\.?|₹)\s*(\d+(?:\.\d+)?)\s*(?:[-–—]|to)\s*(\d+(?:\.\d+)?)\s*(?:LPA|lpa|lakhs?\s*(?:per\s*annum)?|L)/g;
  const lpaRe2 = /(\d+(?:\.\d+)?)\s*(?:[-–—]|to)\s*(\d+(?:\.\d+)?)\s*(?:LPA|lpa|lakhs?\s*per\s*annum)/g;
  let best = null;
  let m;

  while ((m = lpaRe.exec(text)) !== null) {
    const lo = parseFloat(m[1]), hi = parseFloat(m[2]);
    if (lo >= 3 && hi >= lo && hi <= 150) {
      best = `\u20B9${lo}L\u2013${hi}L`;
      break;
    }
  }
  ...
  // Fallback: match USD salary ranges for MNCs posting in India with USD
  if (!best) {
    const usdRe = /\$\s*(\d{1,3}(?:,\d{3})*|\d+)\s*[kK]?\s*(?:[-–—]|to|up\s+to)\s*\$?\s*(\d{1,3}(?:,\d{3})*|\d+)\s*[kK]?/g;
    ...
    <div class="job-card${cardExtra}" data-id="${eid}" data-status="${escapeHtml(j.status)}" data-posted-ts="${postedTimestamp(j.posted_at)}" data-applied-ts="${postedTimestamp(j.applied_at)}" style="--score-color:${scoreColor(sc)}">
      ...
      <div class="job-col-status">
        <select class="pipeline-select" onchange="setPipeline('${eid}', this.value, this)" style="color:${pipelineColor(pval)}">
          ...
          <option value="ghosted" ${pval==='ghosted'?'selected':''} style="color:${COLORS.gray}">Ghosted</option>
        </select>
      </div>
      <div class="job-col-actions">
        ...
        <div class="job-actions-menu" id="jmenu-${eid}">
          ...
          <button class="job-action-item" onclick="openQuestionsCopy('${eid}','${eTitle.replace(/'/g,"\\'")}','${eCompany.replace(/'/g,"\\'")}'); closeJobMenu('${eid}')">${ICON_FILE} Copy Answers</button>
