...
const { saveScreenshot } = require('../ats-appliers/browser');
...
const { parseGreenhouseUrl } = require('../greenhouse-url');
const { sleep, DELAYS } = require('../ats-appliers/utils');
const log = require('../logger')('apply-submit');

function parseGreenhouseApplyTarget(job) {
  return parseGreenhouseUrl(job && job.url, job && job.company);
}
...
async function fillGreenhouseEducation(page, edu) {
  ...
  if (addBtn) { await addBtn.click(); await sleep(DELAYS.DOM_UPDATE); }
...
async function submitGreenhouse(page, job, applicant, answers, tmpResume) {
  ...
  if (countryInput) {
    ...
    if (tag === 'select') {
      await countryInput.select('India');
    } else {
      // React autocomplete: clear, type, then pick the matching option
      await countryInput.click({ clickCount: 3 });
      await countryInput.type('India', { delay: 40 });
      await sleep(DELAYS.DOM_UPDATE);
      const picked = await page.evaluate(() => {
        const items = document.querySelectorAll('[role="option"], li[class*="option"]');
        for (const item of items) {
          if (item.innerText.trim().toLowerCase().includes('india')) {
            item.click(); return true;
          }
        }
        return false;
      });
      if (!picked) log.warn('Could not select India from country dropdown');
      else log.info('Selected country: India');
    }
    await sleep(DELAYS.INPUT_SETTLE);
  } else {
  ...
  if (locationEl && applicant.location) {
    ...
    await sleep(DELAYS.REACT_RENDER); // Wait for Places API suggestions
